...
echo "🔔 重要な設定:"
echo ""
echo "1. HuggingFaceトークン（ターンテイキング機能を使用する場合）:"
echo "   .envファイルを編集してHF_TOKEN=の行にトークンを設定してください"
echo "   トークン取得: https://huggingface.co/settings/tokens"
echo ""
echo "2. 次のステップ:"
if [[ "$OSTYPE" == "darwin"* ]]; then
    echo "   - (macOS) 音声テスト: ./scripts/test_audio.sh"
fi
echo "   - ./scripts/run.sh でコンテナを起動"
echo "   - コンテナ内で ./scripts/start_diaros.sh を実行"
echo ""
